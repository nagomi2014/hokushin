...
interface GuidedDerivationProps {
  onApply: (creed: string) => void;
  onCancel?: () => void;
}

/**
 * 台本式「導き出し対話」（AI不要・費用ゼロ）。
 * 易しい質問に選択肢で答えていくと、価値が見えてきて、最後に理念のドラフトが立ち上がる。
 */
export function GuidedDerivation({ onApply, onCancel }: GuidedDerivationProps) {
  ...
  const q = CREED_QUESTIONS[index];
  const isLast = index >= CREED_QUESTIONS.length - 1;
  const mirror = useMemo(() => mirroredValues(answers), [answers]);
  ...
  function finish(finalAnswers: GuidedAnswer[]) {
    setDraft(synthesizeCreed(finalAnswers));
    setPhase("draft");
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* mirror（2問以上答えたら、見えてきた価値を見せる） */}
      {mirror.length > 0 && answers.length >= 2 && (
